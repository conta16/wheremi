...
function search(callback){
  wmi_search(100, L.userPosition.latLng, {language: $("#language option:selected").val(), audience: $("#audience option:selected").val()}, function(videos){
      ...
      for (var i in tmp_list){
        ...
        sorted_places[i]=sorted_places[i].concat(tmp_list[i].why);//a questo punto tmp_list è un oggetto del tipo [[...video obj...], ...] ordinati per distanza dalla posizione dell'utente di cui, come ha detto vitali, il primo è un what,il secondo un how, poi una lista di why
      }
...
function sort_places(){
  ...
  tmp_list.sort(function(a, b){
    ...
    return facade.distance(L.userPosition.latLng.lat,L.userPosition.latLng.lng, latLngA.lat, latLngA.lng)-facade.distance(L.userPosition.latLng.lat,L.userPosition.latLng.lng, latLngB.lat, latLngB.lng);
  })
  return tmp_list;
}
